...
const Urlwebsocket = 'https://aztrading.info:8008'
export default {
        ...
        if(value){
            const config = {
                headers: { Authorization: `Bearer ${value}` }
                    ...
                    axios.get(`${Urlwebsocket}/login?username=${'tugoftrades'}&password=${'toc__123'+'#'}`, config)
    ...
    searchSymbols: (userInput, exchange, symbolType, onResultReadyCallback) => {
        ...
        axios.get(`${Urlwebsocket}/searchSymbol/?searchKey=${userInput}`)
            .then(function (response) {
                // handle success
   ...
   // retrieve information about a specific symbol (exchange, price scale, full symbol etc.)
    resolveSymbol: (symbolName, onSymbolResolvedCallback, onResolveErrorCallback) => {
        // console.log('[resolveSymbol]: Method call', symbolName);
                ...
                axios.get(`${Urlwebsocket}/resolveSymbol/?symbol=${symbolName}`)
                .then(function (response) {
                    // handle success
                    ...
                    i.intraday_multipliers = ['1', '3', '5', '15', '30', '60', '120', '180', '240', '1D', '1W', '1M']
                    i.has_daily = true
    ...
    getBars: (symbolInfo, resolution, periodParams, onHistoryCallback, onErrorCallback, subscriberUID) => {
        ...
        axios.get(`${Urlwebsocket}/getBars/?symbol=${symbolInfo.ticker}&tf=${resolution}&range=${periodParams.countBack}&to=${periodParams.to}&usr=${userClient.session}`)
        .then((response)=>{
    ...
    subscribeBars: (symbolInfo, interval, onRealtimeCallback, subscriberUID, onResetCacheNeededCallback, resolution) => {
        ...
        if(!rMode){
            ...
            if(rPlayMode){
                axios.get(`${Urlwebsocket}/getReplayBars/?symbol=${symbolInfo.ticker}&tf=${interval}&range=${rPlayRange}&to=${rModeFrom}&usr=${userClient.session}`)
                .then((response)=>{
                    // console.log("====>",response.data)
                    let rbars = [];
                    response.data.map(i => {
